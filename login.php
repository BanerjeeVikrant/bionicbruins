...
if (isset($_POST["user_login"]) && isset($_POST["password_login"])) {
	$user_login = $_POST["user_login"];
    	...
    	$result = $conn->query("SELECT id FROM members WHERE username='$user_login' AND password='$password_login'LIMIT 1"); // query the person
	...
	if ($userCount == 1) {
		$row = $result->fetch_assoc();
             	$id = $row["id"];
		$_SESSION["id"] = $id;
		$_SESSION["bruin_login"] = strtolower($user_login);
...
<head>
	...
	<style type="text/css">
		...
		* { -webkit-box-sizing:border-box; -moz-box-sizing:border-box; -ms-box-sizing:border-box; -o-box-sizing:border-box; box-sizing:border-box; }
		...
		.login h1 { color: #fff; text-shadow: 0 0 10px rgba(0,0,0,0.3); letter-spacing:1px; text-align:center; margin-bottom: 25px;}
...
<body>
	...
	<div id="content">
		...
		<div class="login">
			<h1>Login</h1>
		    <form method="post">
		    	<input type="text" name="user_login" placeholder="Username" required="required" />
		        <input type="password" name="password_login" placeholder="Password" required="required" />
